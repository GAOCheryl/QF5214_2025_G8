...
class DailyBatchSamplerRandom(Sampler):
    def __init__(self, data_source, shuffle=False):
        ...
        # calculate number of samples in each batch   ########### 这里要变Date
...
class SequenceModel():
    ...
    def predict(self, dl_test):
        ...
        for data in test_loader:
            data = torch.squeeze(data, dim=0)
            feature = data[:, :, 0:-1].to(self.device)
            label = data[:, -1, -1] # 本文件中，label代表真实return，提取真实 return，到时根据我们的数据变化
            ######  2
            price = data[:, -1, 1]  # 假设第2个 feature 是价格, 任意选择，到时根据我们的数据变化，比如adjusted closed price
            market = data[:, -1, 2]  # 假设第3个 feature 是market cap, 根据我们数据进行变化
            ######
            ...
            ######   3
            labels.append(label.detach().cpu().numpy())  # 关键：存入 labels
            prices.append(price.detach().cpu().numpy())  # 关键：存入 prices
            markets.append(market.detach().cpu().numpy())  # 关键：存入 markets
            ######
        ...
        real_prices = pd.Series(np.concatenate(prices), index=dl_test.get_index(), name="Price")
        market_cap = pd.Series(np.concatenate(markets), index=dl_test.get_index(), name="Market_Cap")
        ######
