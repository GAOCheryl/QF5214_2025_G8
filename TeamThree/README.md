...
## 3. Model Description
The MASTER model is designed to forecast stock returns using a multi-headed attention mechanism that fuses stock data, alpha factors, market index with sentiment indicators.
...
|-------------|-------------|-----------|---------|
| Stock Data | Stock OHLC, volume and market cap information | Float | Close: 150.23 |
...
The model's outputs are used to construct a trading strategy that selects the top and bottom N stocks based on predicted returns. 
...
The strategy was backtested across four variants by <code>MASTER-master/other strategies.py</code>:
...
The output can be found in <code>MASTER-master/data/Output/TopN_EqualWeight_position.csv, MASTER-master/data/Output/TopN_marketcap_position.csv, MASTER-master/data/Output/TopN_LongShort_position.csv, MASTER-master/data/Output/TopN_LongShort_Marketcap_position.csv </code>
...
- <code>MASTER-master/master_strategy.py</code> : Contains the core model architecture and prediction routines.
- <code>MASTER-master/base_model_strategy.py</code> : Provides basic model implementations.
- <code>MASTER-master/main_sentiment_strategies.py</code> : Implements the data preprocessing, model training, history data predicition in sentiment factor inclusion case.
- <code>MASTER-master/main_without_sentiment_strategies.py</code> : Implements the data preprocessing, model training, history data predicition in sentiment factor exclusion case.
- <code>MASTER-master/final_sentiment_strategy.py</code> : Implements the real-time complete prediction and trading.

## 6. Deployment and Data Storage
### 6.1 CSV & Database Integration
- Updated predictions is stored as <code>MASTER-master/data/Output/updated_prediction.csv</code>.
- The final trading allocations are also inserted into a PostgreSQL database table (tradingstrategy.dailytrading) using SQLAlchemy and CSV table <code>MASTER-master/data/Output/Updated_Allocation.csv</code>.
